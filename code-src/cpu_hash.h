 ...
 * @note   File name changed from parallel_radix_join.c to cpu_hash.h to support other modes
 *
...
const int thresh1 = 2*(args->totalR/args->nthreads); //MAX((1<<D), (1<<R)) * THRESHOLD1(args->nthreads);
...
for(uint32_t ntimes = 0; ntimes < 1; ntimes++)
{
