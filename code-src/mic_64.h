 ...
 *
 *  
 * @brief  Provides various variants of radix hash joins for 64 bit workload on MIC
 * @Note   This file is only for mic cards. Code adapted from ETH source code. 
 * 	   Not all functions tested throughly. test PRO . Updates will be available soon.
 *
...
#include <stdio.h>              /* printf */
      /* simd only for 32-bit keys â€“ SSE4.1 */
...
{
__m512i m512mask = _mm512_set1_epi64(mask); 
k = _mm512_and_epi64(k,m512mask);
__m512 p;
k = _mm512_srli_epi64(k,nbits);
return k;
} 
                     ...
                     relation_t * const tmpR)
{
   ...
   // printf("\n numR:%d",numR);
    ...
    __attribute__ ((align(64))) int64_t extVector[8];
    int64_t *p=(int64_t*)Rtuples;
    const __m512i voffset = _mm512_set_epi64(28,24,20,16,12,8,4,0);
    int upto = (numR>>4)<<4;
    uint32_t i;
   #ifdef SIMD
   for(i=0;i<upto;)
    {
    	...
    	_mm_prefetch((char*)(p+PDIST),_MM_HINT_T0);
     	_mm_prefetch((char*)(p+PDIST+8),_MM_HINT_T0);
     	_mm_prefetch((char*)(p+PDIST+32),_MM_HINT_T1);
    	_mm_prefetch((char*)(p+PDIST+40),_MM_HINT_T1);
	#endif 
    	 key = _mm512_i64extgather_epi64(voffset,(void*)p,_MM_UPCONV_EPI64_NONE ,4,0);;
	
	 key = simd_hash(key,MASK,NUM_RADIX_BITS);
	 _mm512_store_epi64((void*)extVector,key);
	 #pragma unroll(8)
	 for(int j=0;j<8;j+=1)
	 {
   ...
   for(i=0;i<upto;)
	...
	key = simd_hash(key,MASK,NUM_RADIX_BITS);
	 ...
	 for(int j=0;j<8;j+=1)
	 {
	 	for(int hit = bucket[extVector[j]]; hit > 0; hit = next[hit-1]){

           		 if(Stuples[i].key == Rtuples[hit-1].key){
                //		 printf("\n %d %d",Stuples[i].key,Rtuples[hit-i].key);
               			 matches ++;
            		}
        	}
        	i++;
	...
	//printf("\n\n\n");
        for(int hit = bucket[idx]; hit > 0; hit = next[hit-1]){
            ...
            if(Stuples[i].key == Rtuples[hit-1].key){
         //   printf("\n hit :%d", hit-1);
         // printf("\n %d %d",Stuples[i].key,Rtuples[i].key);
...
#warning "NOT IMPLEMENTED, LACK OF HARDWARE SUPPORT"
}
              ...
              int D)
{
    ...
    int64_t *p=(int64_t*)inRel->tuples;
    const __m512i voffset = _mm512_set_epi64(28,24,20,16,12,8,4,0);
...
parallel_radix_partition(part_t * const part) 
{
    ...
    __attribute__ ((align(64))) int64_t extVector[8];
    int64_t *p=(int64_t*)rel;
    __m512i offset = _mm512_set_epi64(28,24,20,16,12,8,4,0);
#ifdef SIMD
    for(i=0;i<num_tuples;i+=8)
    {
	#ifdef PDIST
        _mm_prefetch((char*)(p+PDIST),_MM_HINT_T0);
     	_mm_prefetch((char*)(p+PDIST+8),_MM_HINT_T0);
     	...
     	_mm_prefetch((char*)(p+PDIST+40),_MM_HINT_T1);
        #endif
    	key = _mm512_i64extgather_epi64(offset,p,_MM_UPCONV_EPI64_NONE,4,0);
   ...
   p=(int64_t*)rel;
    ...
    for(i=0;i<num_tuples;)
    {
        ...
        _mm_prefetch((char*)(p+PDIST),_MM_HINT_T0);
     	...
     	_mm_prefetch((char*)(p+PDIST+40),_MM_HINT_T1);
	#endif
    	key = _mm512_i64extgather_epi64(offset,(void*)p,_MM_UPCONV_EPI64_NONE ,4,0);;
...
prj_thread(void * param)
{
    ...
    float ttemp = 2.25*(args->totalR/args->nthreads);
